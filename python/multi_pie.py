def multi_pie(label_list,value_list,args):
    ...
    num = len(label_list)
    plt.rcParams['font.sans-serif'] = ['SimHei']  # 使用指定的汉字字体类型（此处为黑体）
    fig, axj = plt.subplots(nrows=args['row'], ncols=args['col'],
                            figsize=(args['col'] * 4, args['row'] * 3))  # 建立饼图坑
    axes = axj.flatten()  # 子图展平
    if num >= (args['row'] * args['col']) * 2:
        n = num // (args['row'] * args['col'])
    else:
        n = 1
    for ax in range(args['row'] * args['col']):
        label = label_list[ax + 1:ax + 1 + n]
        value = value_list[ax + 1:ax + 1 + n]
        value_sum = sum(value)
        share = []
        for j in value:  # 计算均值
            share.append(round(j / value_sum, 4))
        axes[ax].pie(x=share, labels=label, autopct='%3.1f%%')
